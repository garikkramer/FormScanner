...
public class ManageTemplateFrame extends InternalFrame implements TabbedView {
	...
	private JPanel getFieldListButtonPanel() {
		...
		viewTemplateButton = new ButtonBuilder(orientation)
				.withText(
						FormScannerTranslation
								.getTranslationFor(FormScannerTranslationKeys.VIEW_BUTTON))
				.withToolTip(
						FormScannerTranslation
								.getTranslationFor(FormScannerTranslationKeys.VIEW_BUTTON_TOOLTIP))
				.withActionCommand(FormScannerConstants.VIEW_TEMPLATE)
				.withActionListener(manageTemplateController).setEnabled(false)
				.build();
		...
		JPanel innerPanel = new PanelBuilder(orientation)
				.withLayout(new SpringLayout())
				.add(viewTemplateButton)
				.add(saveTemplateButton)
				.add(cancelTemplateButton)
				.withGrid(1, 3).build();
	...
	public void viewButtonEnabled(boolean b){
		viewTemplateButton.setEnabled(b);
	}
